...
def pair_counter_generator(filename):  # Визначаємо генератор для підрахунку пар букв у словах
    ...
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for line_num, line in enumerate(f, 1):
            if line_num > 100:  # Обмежуємо обробку першими 100 рядками
                break
            ...
            for pair in target_pairs:  # Для кожної з 3 обраних пар виконуємо підрахунок
                ...
                for word in words:  # Перевіряємо кожне слово в рядку
                    count += word.count(
                        pair)  # Додаємо кількість входжень пари в поточному слові (тільки всередині слова)
                results[pair] = count  # Зберігаємо результат для цієї пари в словнику

            yield f"Рядок {line_num}: шукали {target_pairs} -> знайшли {results}"  # Повертаємо форматований рядок з результатами
...
if __name__ == '__main__':
    ...
    print("Результати роботи генератора (перші 100 рядків):")

    my_gen = pair_counter_generator(file_name)  # Створюємо об'єкт генератора для підрахунку пар

    for i in range(100):  # Цикл для виведення перших 100 результатів
        try:
            print(next(my_gen))  # Виводимо результат чергової ітерації генератора
        except StopIteration:  # Якщо генератор вичерпано (менше 100 рядків)
            break
